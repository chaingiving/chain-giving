...
contract CGCrowdfunding is Ownable {
    ...
    constructor(address owner_, uint256 target_, uint256 deadline_) Ownable(owner_) {
    ...
    function _contribute(address donor) internal {
        if (state != State.UNFUNDED) revert NotInState(State.UNFUNDED, state);
    ...
    function withdraw(address to) external onlyOwner {
        if (state != State.FUNDED) revert NotInState(State.FUNDED, state);
    ...
    function cancel() external onlyOwner {
        if (state != State.UNFUNDED && state != State.FUNDED) revert NotInState(State.UNFUNDED, state);
